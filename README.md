...
Drive together on your network: synced vehicles, nametags with live ping, player list and chat.
One person runs the small C++ server, everyone installs the mod.
...
Verified: the server builds and passes a 15-case UDP integration suite driven by fake clients
...
late-joiner world state). Lua and JS files are syntax-checked.

**Not yet verified in-game.** The client half has not been run inside BeamNG.drive on this
machine, so nametag rendering, the dead-reckoning physics correction and the UI app are
code-complete but unproven against a live engine. Expect to iterate on the first run.

## Install

### 1. Server (one machine, the host)

Prebuilt is not shipped; build it once:
...
2. In the LANMP app, enter the host's IP and port.
